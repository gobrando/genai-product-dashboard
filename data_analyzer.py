...
This is a generalized version that reads all product-specific settings
(excluded users, trace type patterns, category keywords, geographic
regions, etc.) from a DashboardConfig object rather than embedding
them in code.
"""
import pandas as pd
import numpy as np
...
class TraceAnalyzer:
    ...
    def __init__(
        ...
        Args:
            spans: Raw span dicts from Phoenix.
            config: A DashboardConfig instance.  If *None* a default config
                    is created (useful for quick / unconfigured usage).
            exclude_user_emails: Additional emails to exclude beyond those
    ...
    def _apply_excluded_user_filter(
        ...
        """
        Remove internal/test users from all analytics by dropping entire
        traces whose user_email matches the exclusion list configured in
        ``self.config`` (plus any extra emails passed in).
        """
        ...
        # Combine config-level exclusions with any extras passed at init
        excluded_emails = {e.lower().strip() for e in (exclude_user_emails or set())}
        excluded_emails |= {e.lower() for e in self.config.excluded_emails}

        emails = self.traces_df['user_email'].astype(str).str.lower().str.strip()
        is_excluded = emails.isin(excluded_emails)

        # Apply domain-pattern exclusion (e.g. r'@navapbc\.[a-z0-9.-]+$')
        for pattern in self.config.excluded_domain_patterns:
    ...
    def _extract_user_email_from_spans(
        ...
        if valid_emails:
            # Prioritize configured domains
            for domain in self.config.priority_email_domains:
    ...
    # ------------------------------------------------------------------
    # Trace-level DataFrame construction
    # ------------------------------------------------------------------
    ...
    def _create_traces_dataframe(self, spans: List[Dict]) -> pd.DataFrame:
        ...
        """
        from config import classify_trace_type, classify_query_category
        ...
        for trace_id, trace_spans in traces.items():
            ...
            # Classify trace type via config mappings
            root_name = root_span.get('name', '')
            trace_type = classify_trace_type(root_name, self.config)

            # Gather query/category/location context from all spans
                                    ...
                                    if start_idx >= 0:
                                        ...
                                        if end_idx > start_idx:
                                            ...
                                            try:
                                                ...
                                                # Fallback: extract any 5-digit zip from reply text
                                                found_zips = re.findall(r'\b\d{5}\b', reply_str)
                                                zip_codes.extend(found_zips)
                    except Exception as e:
                        logger.debug(f"Failed to parse output for zip codes: {e}")

            # Extract zip codes from query text
            if query:
                self._append_zip_from_text(query, zip_codes)

            # Categorize based on query if no explicit category was found
            if query and not category:
                category = classify_query_category(query, self.config)

            # If still no category, try inferring from resources text
            if not category:
                category = self._infer_category_from_resources_via_config(trace_spans)
            ...
            trace_records.append({
                ...
                'all_zip_codes': ','.join(zip_codes[:5]) if zip_codes else '',
    ...
    # ------------------------------------------------------------------
    # Payload / context extraction helpers
    # ------------------------------------------------------------------

    def _parse_attribute_payload(self, attribute_value):
        """
        Normalize Phoenix attribute payloads (which may be nested dicts or
        JSON strings) into Python dictionaries for easier parsing.
        """
            ...
            if payload and isinstance(payload, dict):
                ...
                if isinstance(data, dict):
                    ...
                    if isinstance(prompt_builder, dict):
                        ...
                        if resources_text:
                            result = classify_query_category(resources_text, self.config)
                            if result:
                                return result
        return ''
    ...
    def _extract_trace_context(self, trace_spans: List[Dict]) -> Dict[str, List[str]]:
        """
        Combine context clues from every span in the trace to capture user
        query, selected categories, and preferred locations.
        """
        from config import classify_query_category
        ...
        for span in trace_spans:
            ...
            if payload:
                ...
                # For pipeline-style spans, try to infer category from resources
                if not context['categories'] and isinstance(payload, dict):
                    data = payload.get('data', {})
                    if isinstance(data, dict):
                        prompt_builder = data.get('prompt_builder', {})
                        if isinstance(prompt_builder, dict):
                            resources_text = str(prompt_builder.get('resources', ''))
                            if resources_text:
                                inferred_cat = classify_query_category(resources_text, self.config)
        ...
        # Derive hints directly from the combined query text
        if context['query']:
    ...
    def _get_detected_trace_types(self) -> List[str]:
        """Return the unique trace types detected in the current data."""
        if self.traces_df is None or self.traces_df.empty or 'trace_type' not in self.traces_df.columns:
            return []
        return sorted(self.traces_df['trace_type'].dropna().unique().tolist())

    def get_user_analytics(self) -> pd.DataFrame:
        """
        Get user-level analytics aggregated from traces.
        Dynamically creates count columns for every detected trace type.
        """
        if self.traces_df.empty:
            return pd.DataFrame()

        trace_types = self._get_detected_trace_types()

        # Build the aggregation dict with a lambda that captures trace_types
        def _type_breakdown(x):
            return {t: int((x == t).sum()) for t in trace_types}

        user_stats = self.traces_df.groupby('user_email').agg({
            'trace_id': 'count',
            'trace_type': _type_breakdown,
            'trace_start': ['min', 'max'],
        ...
        user_stats.columns = [
            'user_email', 'total_traces', 'trace_type_breakdown',
        ...
        # Expand trace type breakdown into individual columns
        for t in trace_types:
            col_name = f'{t}_count'
            user_stats[col_name] = user_stats['trace_type_breakdown'].apply(
        ...
        base_cols = ['user_email', 'user_name', 'total_traces']
        type_cols = [f'{t}_count' for t in trace_types]
        tail_cols = ['first_trace', 'last_trace', 'avg_duration_s', 'total_tokens']
        return user_stats[base_cols + type_cols + tail_cols]
    ...
    def get_trace_time_series(self, freq: str = 'H') -> pd.DataFrame:
        ...
        Dynamically adds columns for each detected trace type.
        ...
        # Add per-type counts dynamically
        trace_types = self._get_detected_trace_types()
        for t in trace_types:
            ts = df_time[df_time['trace_type'] == t].groupby(pd.Grouper(freq=freq)).size()
            time_series[f'{t}_count'] = ts.reindex(time_series.index, fill_value=0)

        time_series = time_series.reset_index()

        # Rename base columns
        col_names = ['timestamp', 'trace_count', 'avg_duration', 'total_tokens']
        col_names += [f'{t}_count' for t in trace_types]
        time_series.columns = col_names
    ...
    def get_resource_category_breakdown(self) -> Dict:
        ...
        for category, count in category_counts.items():
            ...
            category_breakdown[category] = {
                ...
                'examples': category_traces.get(category, [])[:3]
            }
    ...
    def extract_zip_codes(self) -> Dict:
        ...
        if not zip_codes and 'query' in self.traces_df.columns:
            for idx, row in self.traces_df.iterrows():
                ...
                found_zips = re.findall(r'\b\d{5}\b', query)
                zip_codes.extend(found_zips)
        ...
        region_zips = get_region_zip_set(self.config)
    ...
    def get_cohort_analysis(self, cohort_emails: List[str], cohort_name: str = "Cohort") -> Dict:
        """
        Analyze usage for a specific cohort of users.
        ...
        for cohort_email in cohort_emails:
            ...
            # Method 2: Match by username only
    ...
    def get_comprehensive_report(self, start_date=None, end_date=None, cohort_emails=None) -> Dict:
        ...
        # Build dynamic totals from actual trace types
        totals: Dict = {
            'unique_users': filtered_df['user_email'].nunique(),
            'total_traces': len(filtered_df),
        }
        if 'trace_type' in filtered_df.columns:
            for t in filtered_df['trace_type'].unique():
                totals[t] = int(len(filtered_df[filtered_df['trace_type'] == t]))
    ...
    def analyze_query_patterns(self) -> Dict:
        """
        Query Pattern Analysis & Search Intelligence.
        Analyzes query patterns to understand what users need.
        """
        ...
        # Unmet needs: detect users who used "search" type but not "act" type
        # Use the first two configured trace types as search/act if available,
        # otherwise use any detected types.
        trace_types = self._get_detected_trace_types()
        search_type = trace_types[0] if len(trace_types) >= 1 else None
        act_type = trace_types[1] if len(trace_types) >= 2 else None

        users_no_action: set = set()
        no_action_queries = pd.DataFrame()
        if search_type and act_type:
            search_users = set(queries_df[queries_df['trace_type'] == search_type]['user_email'])
            act_users = set(self.traces_df[self.traces_df['trace_type'] == act_type]['user_email'])
            users_no_action = search_users - act_users
        ...
        return {
            ...
            'no_action_examples': no_action_queries[['user_email', 'query', 'category']].head(5).to_dict('records') if not no_action_queries.empty else []
        }
    ...
    def analyze_resource_effectiveness(self) -> Dict:
        ...
        Generalised: uses the first two configured trace types as "search"
        and "act" steps for conversion analysis.  If only one type exists
        the method still reports per-category breakdown.
        """
        if self.traces_df.empty:
            return {}
        ...
        # Fall back gracefully when we have fewer than two types
        search_df = self.traces_df[self.traces_df['trace_type'] == search_type] if search_type else pd.DataFrame()
        act_df = self.traces_df[self.traces_df['trace_type'] == act_type] if act_type else pd.DataFrame()

        if act_df.empty and search_df.empty:
            return {'message': 'Insufficient trace type data to analyze effectiveness'}

        # Demand by category (from "act" traces)
        resources_by_category: Dict = defaultdict(lambda: defaultdict(int))
        if not act_df.empty:
            for idx, row in act_df.iterrows():
                category = row.get('category', 'Other')
                resources_by_category[category]['act_traces'] += 1

        # User conversion: users who searched vs. users who acted
        users_searched = set(search_df['user_email'].unique()) if not search_df.empty else set()
        users_acted = set(act_df['user_email'].unique()) if not act_df.empty else set()
        ...
        for category in self.traces_df['category'].unique():
            ...
            cat_search = len(search_df[search_df['category'] == category]) if not search_df.empty else 0
            cat_act = len(act_df[act_df['category'] == category]) if not act_df.empty else 0
            cat_conversion = (cat_act / cat_search * 100) if cat_search > 0 else 0
            ...
            if search_type:
                row_data[search_type] = cat_search
            if act_type:
                row_data[act_type] = cat_act
            category_effectiveness.append(row_data)
        ...
        result: Dict = {
            ...
            'top_categories_by_action': sorted(resources_by_category.items(),
                                               key=lambda x: x[1].get('act_traces', 0),
                                               reverse=True)[:10]
        }
        if search_type:
            result[f'total_{search_type}'] = len(search_df)
        if act_type:
            result[f'total_{act_type}'] = len(act_df)
    ...
    def analyze_user_journeys(self) -> Dict:
        """
        User Journey & Workflow Analytics.
        ...
        if search_type and act_type:
            for user, user_traces in user_groups:
                search_traces = user_traces[user_traces['trace_type'] == search_type]
                act_traces = user_traces[user_traces['trace_type'] == act_type]

                if not search_traces.empty and not act_traces.empty:
                    first_search = search_traces['trace_start'].min()
                    first_act = act_traces['trace_start'].min()

                    if first_act > first_search:
                        time_diff = (first_act - first_search).total_seconds() / 60
                        time_to_action.append({
                            'user': user,
                            'time_minutes': round(time_diff, 1),
                            f'{search_type}_count': len(search_traces),
                            f'{act_type}_count': len(act_traces)
                        })
        ...
        # Drop-off analysis
        users_dropped: set = set()
        if search_type and act_type:
            users_dropped = (
                set(self.traces_df[self.traces_df['trace_type'] == search_type]['user_email']) -
                set(self.traces_df[self.traces_df['trace_type'] == act_type]['user_email'])
            )
    ...
    def analyze_geographic_gaps(self) -> Dict:
        ...
        Identifies service deserts and regional coverage issues using
        config-driven region definitions and zip-to-city mappings.
        """
        if self.traces_df.empty:
            return {}
        ...
        for z, count in zip_counts.items():
            ...
            if region_zips and z in region_zips:
                in_region[z] = zip_data
            elif region_zips:
                out_of_region[z] = zip_data
            else:
                # No regions configured: everything is "in region"
                in_region[z] = zip_data
        ...
        category_by_region: Dict = {
            'in_region': defaultdict(int),
            'out_of_region': defaultdict(int)
        }

        for z in all_zips:
            bucket = 'in_region' if (not region_zips or z in region_zips) else 'out_of_region'
            cat = zip_to_categories.get(z, ['Other'])[0] if z in zip_to_categories else 'Other'
            category_by_region[bucket][cat] += 1
        ...
        # Build dynamic coverage_summary from configured regions
        coverage_summary: Dict = {}
        for geo_region in self.config.geographic_regions:
            region_key = geo_region.name.lower().replace(' ', '_')
            coverage_summary[region_key] = sum(
                1 for z in zip_counts.keys() if z in set(geo_region.zip_codes)
            )
    ...
    def analyze_comparative_periods(self, period1_start: datetime = None, period1_end: datetime = None,
        ...
        def period_metrics(df):
            ...
            for t in trace_types:
                metrics[t] = int(len(df[df['trace_type'] == t]))
            return metrics
        ...
        for t in trace_types:
            changes[f'{t}_change'] = calc_change(p1_metrics.get(t, 0), p2_metrics.get(t, 0))
        ...
        return {
            ...
            'new_categories': list(p2_cats - p1_cats),
            'dropped_categories': list(p1_cats - p2_cats),
    ...
    def analyze_realtime_alerts(self) -> Dict:
        ...
        # 8. Geographic outliers (config-driven)
        region_zips = get_region_zip_set(self.config)
        if region_zips:
            out_of_region = 0
            total_with_zip = 0
            ...
            if total_with_zip > 0 and out_of_region / total_with_zip > 0.2:
                alerts.append({
                    ...
                    'severity': 'info',
                    'message': f'{out_of_region} requests from outside configured region',
                    'value': out_of_region,
    ...
    def analyze_query_intelligence(self) -> Dict:
        ...
        for idx, row in queries_df.iterrows():
            ...
            # Extract locations (zip codes always)
            locations_found = []
            ...
            trace_types = self._get_detected_trace_types()
            act_type = trace_types[1] if len(trace_types) >= 2 else None
            if act_type and row.get('trace_type') == act_type:
                query_analysis['intent'] = f'create_{act_type}'
            ...
            complexity = 0
            complexity += min(len(query_analysis['demographics']) * 2, 4)
            complexity += min(len(query_analysis['services']) * 1.5, 3)
            complexity += min(len(query_analysis['locations']), 2)
            complexity += 1 if query_analysis['urgency'] in ['critical', 'high'] else 0
        ...
        successful_patterns = []
        for aq in analyzed_queries:
            if aq['quality_score'] >= 70 and aq['services']:
        ...
        return {
            ...
            'all_analyzed': analyzed_queries[:50],
    ...
    def analyze_output_quality(self, return_full: bool = False) -> Dict:
        ...
        return {
            ...
            'issues': {k: v[:5] for k, v in issues.items()},
            'insights': insights,
            'all_analyses': (
    ...
    def _build_output_analyses(self) -> List[Dict]:
        ...
        for trace_id in self.traces_df['trace_id'].unique():
            ...
            analysis['output_text'] = output_text[:5000]
            analysis['resources_found'] = resources_found
    ...
    def _score_output_quality(self, output_text: str, resources: List, query: str,
        ...
        if resource_count > 0:
            ...
            # Check if the trace type is a "search" type (first configured type)
            trace_types = self._get_detected_trace_types()
            search_type = trace_types[0] if trace_types else None
            if search_type and trace_type == search_type:
                score -= 15
                factors.append(f"No resources in {search_type} response")
        ...
        if query_location or query_zip:
            ...
            location_aligned = location_found or not query_location
        ...
        # 5. Category alignment (use config categories for keyword matching)
        category_aligned = True
        if query_category and query_category.lower() != 'other':
            # Build keyword list from config categories
            cat_keywords = []
            for cat_config in self.config.categories:
                if cat_config.name.lower() == query_category.lower():
                    cat_keywords = [kw.lower() for kw in cat_config.keywords]
                    break

            if not cat_keywords:
                # Fallback: use the category name itself as a keyword
                cat_keywords = [query_category.lower()]

            category_found = any(kw in output_lower for kw in cat_keywords)
        ...
        # 6. Generic/template response detection
        generic_phrases = [
        ...
        # 7. Structure quality (for "act" type traces)
        trace_types = self._get_detected_trace_types()
        act_type = trace_types[1] if len(trace_types) >= 2 else None
        if act_type and trace_type == act_type:
            has_steps = bool(re.search(r'(step\s*\d|1\.|bullet|--\s+\w)', output_lower))
    ...
    def _generate_output_insights(self, analyses: List, issues: Dict) -> List[str]:
        ...
        if no_resources_pct > 30:
            insights.append(f"{no_resources_pct:.0f}% of responses contain no resources - expand resource database")
    ...
    def analyze_resource_recommendations(self) -> Dict:
        ...
        # Geographic analysis using config
        region_zips = get_region_zip_set(self.config)
        if region_zips:
            in_region_zips_list = [z for z in zip_distribution.keys() if z in region_zips]
            out_of_region_zips_list = [z for z in zip_distribution.keys() if z not in region_zips]
        else:
            in_region_zips_list = list(zip_distribution.keys())
            out_of_region_zips_list = []
        ...
        return {
            ...
            'geographic_distribution': {
                ...
                'in_region_count': sum(zip_distribution[z] for z in in_region_zips_list),
                'out_of_region_count': sum(zip_distribution[z] for z in out_of_region_zips_list),
                'out_of_region_zips': out_of_region_zips_list
            },
            ...
            'insights': insights,
            'sample_resources': all_resources[:30]
        }

    def _extract_resources_from_text(self) -> Dict:
        """Fallback: Extract resource mentions from output text."""
    ...
    def _run_failure_taxonomy(
        ...
        for analysis in output_analyses:
            ...
            # No resources check (for search-type traces)
            if search_type and analysis.get('trace_type') == search_type and analysis.get('resource_count', 0) == 0:
                output_text_check = analysis.get('output_text', '').lower()
                output_length = analysis.get('output_length', 0)

                resource_indicators = [
                    'address', 'phone', 'website', 'www.', 'http', '.org', '.com',
                ...
                if output_length < 200 or not has_resource_content:
                    ...
                    failures['no_resources'].append({
                        'trace_id': analysis.get('trace_id'),
                        'query': analysis.get('query', ''),
                        'reason': f'{search_type} query returned no resources',
                    })
        ...
        return {
            ...
            'failures_by_type': {k: v[:20] for k, v in failures.items()},
    ...
    def _run_resource_validation(
        ...
        return {
            ...
            'unmatched_resources': unmatched_resources[:50],
    ...
    def _generate_exec_recommendations(
        ...
        for failure_type, count in top_failures:
            if failure_type == 'hallucination' and count > 0:
                recs.append(f"{count} potential hallucinations detected - verify resource inventory")
    ...
    def run_consistency_analysis(
        ...
        Analyzes variance in existing traces with similar queries.
        """
